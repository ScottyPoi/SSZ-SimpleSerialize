...
export function computeProofBitstrings(
  ...
  const path = GindexBitstring();
  const branch = GindexBitstring();
  let g = gindex;
  while (g.length > 1) {
    path.add(g);
 ...
 */
export function computeMultiProofBitstrings(gindices) {
  // Initialize the proof indices with the leaves
  const proof = GindexBitstring(filterParentBitstrings(gindices));
  const paths = GindexBitstring();
  const branches = GindexBitstring();
